...
class OBS_DispatcherRule{
   protected:
      bool isSet[6]; //!< Define whether fields are taken into account or not. Each field's indexes are commented below.
...
//      IPAddress destAddr; //!< Destination address (isSet[1]).
      ...
      IPv4Address destNetworkAddr; //!< Destination address (isSet[5]).
      IPv4Address subnetAddr; //!< Destination address (isSet[5]).
   ...
   public:
      ...
      bool IsIPInRange(const string, const string, const string);
