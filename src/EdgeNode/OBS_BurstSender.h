...
class INET_API OBS_BurstSender : public cSimpleModule{
   protected:
        ...
        int numChannelFastSwitch; // Number of output data channels fast switch path
        int numChannelSlowSwitch; // Number of output data channels slow switch path
      ...
      //  simtime_t sendPacketTime;
        IPv4Datagram *tempPack;


        static simsignal_t sentBurstsFastPathSignal;
        static simsignal_t sentBurstsSlowPathSignal;
        static simsignal_t sentControlPkSignal;
        static simsignal_t rcvdControlPkSignal;
        static simsignal_t rcvdControlPkBidSignal;
        static simsignal_t NegativeDelay;

//        static simsignal_t sentHalfBurstsFastPathSignal;
 //       static simsignal_t sentHalfBurstsSlowPathSignal;

        //    simtime_t *horizon_slow; //!< This array shows the simtime where each data channel is free.
     //   vector< cOutVector* > horizonVecSlow; //!< Horizon value vector. One for each lambda.

        //simtime_t *horizon_fast; //!< This array shows the simtime where each data channel is free.
       // vector< cOutVector* > horizonVecFast; //!< Horizon value vector. One for each lambda.
...
//        int burstDroppedByOffset; //!< Number of bursts dropped because they reached the minimum offset.
   ...
   private:
         int findNearestHorizonSlowSwitch();
         int findNearestHorizonFastSwitch();
};
